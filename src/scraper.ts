...
export async function scrapeMenu(restaurantName: string): Promise<ScrapeResult> {
  ...
  try {
    ...
    for (const link of linkHandles) {
      const [href, text, aria] = await Promise.all([
        link.evaluate((el: any) => (el.getAttribute('href') || '')),
        link.evaluate((el: any) => (el.textContent || '').trim()),
        link.evaluate((el: any) => (el.getAttribute('aria-label') || '').trim())
      ]);
    ...
    if (!clicked && linkHandles.length > 0) {
      await linkHandles[0].evaluate((el: any) => {
        if (el instanceof HTMLElement) {
          el.scrollIntoView({ block: 'center' });
        }
      }).catch(() => {});
    ...
    // Scrape menu items
    const menu = await page.evaluate((): MenuItem[] => {
      const normalize = (s: string): string => (s || '').toLowerCase().trim();
      const priceLike = (s: string): boolean => /\d+(?:[.,]\d{1,3})/.test(s);
      
      const items: MenuItem[] = [];
      
      // Find all menu categories
      const categories = Array.from(document.querySelectorAll('div[data-testid="menu-category"]')) as Element[];
      
      for (const category of categories) {
        // Find all items in this category (inside .content.open)
        const contentOpen = category.querySelector('.content.open') as Element | null;
        if (!contentOpen) continue;
        
        // Find all clickable item containers
        const itemContainers = Array.from(contentOpen.querySelectorAll('div.clickable, div[class*="clickable"]')) as Element[];
        
        for (const itemContainer of itemContainers) {
          // Extract name from .item-name > .f-15
          const nameEl = itemContainer.querySelector('.item-name .f-15, .item-name > .f-15') as Element | null;
          const name = nameEl?.textContent?.trim() || '';
          
          if (!name) continue;
          
          // Extract description from .item-name > .f-12.description
          const descEl = itemContainer.querySelector('.item-name .f-12.description, .item-name .description, .item-name > .description') as Element | null;
          ...
          const priceContainer = itemContainer.querySelector('.text-right.price-rating') as Element | null;
          if (!priceContainer) continue;
          
          // Find all currency elements, but skip the one with old-price
          const allCurrencyEls = Array.from(priceContainer.querySelectorAll('.currency')) as Element[];
          let priceRaw = '';
      ...
      if (!items.length) {
        const itemContainers = Array.from(document.querySelectorAll('div.clickable, div[class*="clickable"]')) as Element[];
        for (const itemContainer of itemContainers) {
          const nameEl = itemContainer.querySelector('.item-name .f-15, .item-name > .f-15') as Element | null;
          const name = nameEl?.textContent?.trim() || '';
          ...
          const descEl = itemContainer.querySelector('.item-name .description, .item-name > .description') as Element | null;
          const desc = descEl?.textContent?.trim() || '';
